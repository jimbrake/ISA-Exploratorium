--		vga_video2.vhd		VGA controller for BCD_1
--		generates video display for 1000 digit memory
--		with VGA timing (~25.175 Mhz clock):
--		horizontal timing in clocks: 640, 24, 96, 48 (808 total)
--			active, back porch, sync, front porch (25.17,0.94,3.77,1.89us = 31.77us total)
--		vertical timing in scan lines: 480, 11, 2, 32 (525 total)
--			active, back porch, sync, front porch (15.24,0.35,0.064,1.02ms = 16.6ms total)
...
--	h reg
			if (h(9)='1')and(h(8)='1')and(h(5)='1')and(h(3)='1') then h <= "0000000001"; else h <= h + 1; end if;
-- v reg
			if (h(9)='1')and(h(8)='1')and(h(5)='1')and(h(3)='1') AND 
				(v(9)='1')and(v(3)='1')and(v(2)='1')and(v(0)='1') then v <= "0000000001"; else v <= v + 1; end if;
-- hblk
			if (h(9)='1')and(h(7)='1') then hblk <= '1';
			elsif (h(9)='0') then hblk <= '0'; end if;
-- vblk
			if (v(9 downto 4)="01111")and(v(0)='1') then vblk <= '1';
			elsif (v(9 downto 8)="00") then vblk <= '0'; end if;
-- hs
			if (h(9)='1')and(h(7)='1')and(h(4)='1')and(h(3)='1') then hs <= '1';
			elsif (h(9)='1')and(h(7 downto 3)="1111") then hs <= '0'; end if;
--	vs
			if (v(8 downto 5)="1111")and(v(3)='1')and(v(1)='1')and(v(0)='1') then vs <= '1';
			elsif (v(8 downto 5)="1111")and(v(3)='1')and(v(2)='1')and(v(0)='1') then vs <= '0'; end if;
-- ma reg
			if (h(3 downto 0)="0001")AND(v(3 downto 0)="0001")AND(vblk = '1') then ma <= "000000000000";
				else ma <= mm;
				end if;
--	data reg
			if (h(3 downto 0)="0001")AND(v(3 downto 0)="0001") then d <= vdata; end if;
-- intensify every 10th digit
			if (ma(3 downto 0)="0000")AND(blnkn = '1') then g(0) <= '1'; else g(0) <= '0'; end if;
--	digit pixels
